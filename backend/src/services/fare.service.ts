...
/**
 * Calculate metro fares for both lines given a straight-line
 ...
 */
export function calculateMetroFare(distanceKm: number): MetroFareBreakdown[] {
    ...
    for (const line of CHENNAI_METRO_LINES) {
        const stationCount = Math.min(estimatedStations, line.stations.length);

        const slab = METRO_FARE_SLABS.find(s => stationCount <= s.maxStations);
        if (!slab) continue;

        const durationMin = Math.round(stationCount * line.avgInterStationMin) + 5; // +5 for walking/waiting
 ...
 * the booking service needs a single number.
 */
